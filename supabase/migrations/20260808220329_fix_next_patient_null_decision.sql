-- Migration 0030: fix NULL propagation in next_patient().
--
-- (p_long_decision = 'break') yields NULL when p_long_decision is NULL —
-- SQL three-valued logic, not false — which violated the NOT NULL
-- constraint on exclude_from_prediction and broke the ordinary path
-- (the one where the nurse is asked nothing at all). The comparison
-- needed to be explicitly coalesced.
